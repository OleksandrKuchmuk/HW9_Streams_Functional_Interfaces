...
public class Actions {
    ...
    public  List<Integer> alignTheList(List<List<Integer>> inputList) {
    ...
    public  List<Integer> addElementToList(List<Integer> list, Integer e) {
        list.add(e);
        return list;
    }

    public  List<Integer> changeNull(List<Integer> list) {
        return list.stream()
                .map(x -> Optional.ofNullable(x).orElse(55))
                .collect(Collectors.toList());
    }

    public List<Integer> numberMultiplication(List<Integer> list, int multiplier) {
